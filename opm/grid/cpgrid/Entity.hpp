...
//          }
        ...
        private:
            ...
            /// \brief stores the corner of the father if geometryInFather is called.
            mutable std::shared_ptr<EntityVariable<cpgrid::Geometry<0, 3>, 3>> father_corners_;
            ...
            /// \brief Indices of corners in father_corners_ for the Geometry returned by geometryInFather
            static constexpr std::array<int,8> father_corner_indices_ =
                { 0, 1, 2, 3, 4, 5, 6, 7 };
        };
...
Dune::cpgrid::Geometry<3,3> Dune::cpgrid::Entity<codim>::geometryInFather() const
{
    if (!(this->hasFather())){
        ...
        FieldVector<double, 3> corners_in_father_reference_elem_temp[8] = {
            ...
            { double(eIJK[0]-child0_IJK[0]+1)/cells_per_dim[0], double(eIJK[1]-child0_IJK[1]+1)/cells_per_dim[1],
              double(eIJK[2]-child0_IJK[2]+1)/cells_per_dim[2] }};
        father_corners_ = std::make_shared<EntityVariable<cpgrid::Geometry<0, 3>, 3>>();
        EntityVariableBase<cpgrid::Geometry<0, 3>>& mutable_father_corners = *father_corners_;
        // assign the corners. Make use of the fact that pointers behave like iterators.
        mutable_father_corners.assign(corners_in_father_reference_elem_temp,
        ...
        return Dune::cpgrid::Geometry<3,3>(center_in_father_reference_elem, volume_in_father_reference_elem,
                                           *father_corners_, father_corner_indices_.data());
    }
